    ...
    return (
        <div className="space-y-6">
            ...
            <section className="rounded-2xl border border-black/10 bg-white p-6">
                <h2 className="mb-4 text-lg font-medium text-[#0B0B0B]">Kontakt</h2>
                <div className="grid gap-4 sm:grid-cols-2">
                    <Field label="E-Mail" type="email" value={data.email} onChange={(v) => set("email", v)} placeholder="info@romabeauty.de" />
                    <Field label="Telefon" value={data.phone} onChange={(v) => set("phone", v)} placeholder="+49 123 456789" />
                    <Field label="Straße & Nr." value={data.street} onChange={(v) => set("street", v)} placeholder="Musterstraße 15" />
                    <div className="grid grid-cols-3 gap-3">
                        <Field label="PLZ" value={data.zip} onChange={(v) => set("zip", v)} placeholder="59555" />
                        <div className="col-span-2">
                            <Field label="Ort" value={data.city} onChange={(v) => set("city", v)} placeholder="Lippstadt" />
                        </div>
                    </div>
                    <Field label="Land" value={data.country} onChange={(v) => set("country", v)} placeholder="Deutschland" />
                </div>
            </section>

            {/* RECHTLICHES */}
            <section className="rounded-2xl border border-black/10 bg-white p-6">
                <h2 className="mb-1 text-lg font-medium text-[#0B0B0B]">Rechtliches (Impressum)</h2>
                <p className="mb-4 text-sm text-gray-500">Erscheint auf der Impressum-Seite.</p>
                <div className="grid gap-4 sm:grid-cols-2">
                    <Field label="Geschäftsführer" value={data.managingDirector} onChange={(v) => set("managingDirector", v)} placeholder="Max Mustermann" />
                    <Field label="Registergericht" value={data.registerCourt} onChange={(v) => set("registerCourt", v)} placeholder="Amtsgericht Paderborn" />
                    <Field label="Handelsregisternummer" value={data.hrb} onChange={(v) => set("hrb", v)} placeholder="HRB 12345" />
                    <Field label="USt-IdNr." value={data.vatId} onChange={(v) => set("vatId", v)} placeholder="DE123456789" />
                </div>
            </section>
            ...
            <section className="rounded-2xl border border-black/10 bg-white p-6">
                <div className="mb-4 flex items-center justify-between">
                    <h2 className="text-lg font-medium text-[#0B0B0B]">Social Media</h2>
                ...
                {data.social.length === 0 ? (
                    <p className="text-sm text-gray-500">Noch keine Links. Beliebig viele möglich (Instagram, TikTok, Facebook, WhatsApp …).</p>
                ) : (
