        ...
        <div>
            <div className="grid gap-6 lg:grid-cols-[260px_1fr]">
                ...
                <div className="space-y-2 lg:sticky lg:top-20 self-start">
                    <p className="mb-2 text-xs uppercase tracking-wide text-gray-400">Vorlagen</p>
                    {TEMPLATE_META.map((m) => {
                        const t = templates[m.key];
                        ...
                        return (
                            ...
                            >
                                <div className="flex items-center justify-between gap-2">
                                    <p className="text-sm font-medium text-[#0B0B0B]">{m.label}</p>
                                    <span
                                        aria-hidden
                                        className={`h-2.5 w-2.5 shrink-0 rounded-full ${t.enabled ? "bg-[#C8A24A]" : "bg-gray-300"}`}
                                    />
                                </div>
                                <p className="mt-0.5 text-xs text-gray-500">
                                    {m.audience === "customer" ? "An Kunden" : "An Studio"}
                ...
                <div className="space-y-6">
                    <section className="rounded-2xl border border-black/10 bg-white p-5 sm:p-6">
                        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-black/10 pb-4">
                            <div className="min-w-0">
                                <h2 className="text-lg font-medium text-[#0B0B0B]">{meta.label}</h2>
                            ...
                            <label className="flex shrink-0 items-center gap-2 text-sm text-gray-600">
                                <span>{current.enabled ? "Aktiv" : "Aus"}</span>
                                <button
                                    type="button"
                                    onClick={toggleEnabled}
                                    aria-label={current.enabled ? "Vorlage deaktivieren" : "Vorlage aktivieren"}
                    ...
                    <section className="rounded-2xl border border-black/10 bg-white p-5 sm:p-6">
                        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                            <h3 className="text-sm font-medium text-[#0B0B0B]">Vorschau</h3>
                            <span className="text-xs text-gray-400">Beispieldaten</span>
                        </div>
