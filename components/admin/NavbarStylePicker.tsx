...
export default function NavbarStylePicker({ initial }: { initial: NavbarStyle }) {
    ...
    async function pick(next: NavbarStyle) {
        ...
        try {
            ...
            setStatus({ ok: true, text: t("Gespeichert ✓ – live auf der Seite") });
        } catch {
            setStyle(prev);
            setStatus({ ok: false, text: t("Konnte nicht gespeichert werden.") });
        } finally {
            setBusy(false);
        }
    }
    ...
    return (
        <div className="mb-6 rounded-2xl border border-black/10 bg-white p-5">
            <div className="mb-1 flex flex-wrap items-center justify-between gap-2">
                <h3 className="text-sm font-medium text-[#0B0B0B]">{t("Navbar-Stil")}</h3>
                {status && (
                    <span className={`text-xs ${status.ok ? "text-green-600" : "text-red-600"}`}>{status.text}</span>
                )}
            </div>
            <p className="mb-4 text-xs text-gray-500">
                {t("Schriftzug der oberen Navigation – greift sofort live. Wird nur angezeigt, solange kein eigenes Logo-Bild hochgeladen ist. Der Gold-Schimmer bewegt sich dauerhaft.")}
            </p>
                ...
                {NAVBAR_STYLES.map((s) => {
                    ...
                    return (
                        <button
                            ...
                            disabled={busy}
                            aria-pressed={active}
                            className={`rounded-xl border p-4 text-start transition disabled:opacity-60 ${
                                active
                        ...
                        >
                            ...
                            <p className="text-xs text-gray-500">{t(s.hint)}</p>
                            {active && <p className="mt-1 text-xs font-medium text-[#C8A24A]">{t("Aktiv")}</p>}
                        </button>
                    );
                })}
            </div>
