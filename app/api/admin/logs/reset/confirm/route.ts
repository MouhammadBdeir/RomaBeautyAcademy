// Bestätigt das Zurücksetzen des Protokolls über den Token aus der Owner-Mail.
// Öffentlich erreichbar (der Owner klickt aus der E-Mail) – abgesichert über den Token.
import { adminDb } from "@/lib/firebase/admin";
import { addLog, clearLogs } from "@/lib/logs/server";
...
export async function GET(request: Request) {
    ...
    const ref = adminDb().collection("config").doc("logsResetPending");
    ...
    if (createdMs && Date.now() - createdMs > 24 * 60 * 60 * 1000) {
        ...
        return page("Link abgelaufen", "Bitte das Zurücksetzen im Admin-Bereich erneut anfragen.", false);
    }
    ...
    await addLog({
        category: "admin",
        level: "warn",
        message: "Protokoll per Owner-Bestätigung zurückgesetzt",
        actor: (data.requestedBy as string) ?? "Owner",
    });

    return page("Protokoll zurückgesetzt ✓", "Alle Protokoll-Einträge wurden gelöscht. Du kannst dieses Fenster schließen.", true);
}
